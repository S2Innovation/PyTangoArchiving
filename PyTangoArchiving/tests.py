...
class EvalReader(PyTangoArchiving.Reader):
  def get_attribute_values(self,attribute,start_date,stop_date=None,asHistoryBuffer=False,N=0):
    ...
    if is_eval:
      ...
      vals = dict((k,fandango.arrays.filter_array(v,window=resolution)) for k,v in list(self.get_attributes_values([a.strip('{}') for a in attributes],start_date,stop_date).items()))
      cvals = self.correlate_values(vals,resolution=resolution,rule=(lambda t1,t2,tt:t2))
      nvals = []
      for i,t in enumerate(list(cvals.values())[0]):
       try:
        vars = dict((getId(k),v[i][1]) for k,v in list(cvals.items()))
        if None in list(vars.values()):
         v= None
        else:
