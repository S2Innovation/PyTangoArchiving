...
def loadCSVfile(filename,dedicated={},deletelist=[],context={}):
  ...
  for p,v in list(defaultparams.items()):
    if p not in list(hosts.keys()) or not hosts[p] or list(hosts[p].keys())[0]==v:
      raise Exception('PARAMS_ERROR','All these defaultparams are MANDATORY!: %s'%str(list(defaultparams.keys())))
    ...
    for dev,attributes in sorted(devs.items()):
      ...
      if template:
        if ':' in template[0]:
            ...
            dev2 = list(attributes[template[0]].keys())[0]
        #else: dev2 = ''
        [attributes.__setitem__(k,v) for k,v in list(devs[dev2].items()) if k and k not in attributes]
        [attributes.pop(t) for t in template]
      
      #If a @DELETE or @STOP is found as single attribute all dev. attributes are stopped
      elif '@DELETE' in list(attributes.keys()) or '@STOP' in list(attributes.keys()):
      ...
      for attribute,modes in sorted(attributes.items()):
        ...
        if not modes:
          ...
          firstmode,firstparam=list(modes.keys())[0],list(modes.values())[0][0]
...
def splitAttributesInGroups(attrslist,MAX_SIZE=100):
  ...
  if len(attrslist)>MAX_SIZE:
    ...
    for domain,attributes in list(domains.items()):
      if len(attributes)<=MAX_SIZE:
        result[domain]=list(attributes.keys())
      else:
        ...
        for family,atts in list(families.items()):
          for i in range(1+int(len(atts)/100)):
            ...
            result[gid]=list(atts.keys())[100*i:100*(i+1)]
  else: result['ALL']=list(attrslist.keys())
  return result
...
def configureArchivers(api,arch_type,attrslist,dedicated,delete_list):
  ...
  previous_archiver={}
  for a in list(api.attributes.values()):
  ...
  atts = [a for a in list(attrslist.keys()) if a not in delete_list]
  for a in atts:
    ...
    if a.lower() in list(previous_archiver.keys()) and re.match(reg,previous_archiver[a.lower()].lower()):
      ...
      elif dev in list(dedicated.keys()):
      ...
      if dev in list(dedicated.keys()):
        ...
        for arch,d in list(archiver_properties.items()):
    ...
    if arch_type.lower()=='tdb':
      if archiver in list(archiver_properties.keys()):
