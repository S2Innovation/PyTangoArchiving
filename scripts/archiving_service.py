...
if 'status' in sys.argv:
    ...
    running = sorted(['\t'.join((d.name,d.host,str(d.level))) for d in list(astor.values()) if d.state == PyTango.DevState.ON and (d.host or d.level)])
    failed = sorted(['\t'.join((d.name,d.host,str(d.level))) for d in list(astor.values()) if d.state != PyTango.DevState.ON and (d.host or d.level)])
